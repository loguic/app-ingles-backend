"""Run one explicitly configured B38–B52 source-integrity operation."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run one explicit B38–B52 source-integrity operation.",
        allow_abbrev=False,
    )
...
def _shared_b39(result: ActiveCandidateSourceIntegrityVerification) -> bool:
    b43 = (
...
def _success_output(result: ActiveCandidateSourceIntegrityVerification) -> str:
    b51 = result.resource_integrity_verification
    source_revision = (
        b51.observed_resource_identity_collection.resource_acquisition
        .resource_binding_collection.expected_resource_coverage_verification
        .required_resource_inventory.candidate_integrity_verification.snapshot
        .snapshot_revision
    )
    resource_count = len(b51.observed_resource_identity_collection.entries)
    return urlencode(
        (
...
def _report_text(args: argparse.Namespace, output: str) -> str:
    return "\n".join(
        (
            "# A1 B52 Controlled Source Integrity Run",
            "",
            "## Explicit inputs",
            "",
            ...
            output,
            "",
            "No activation, loader, runtime, content-tree, or verification-state persistence occurs through this CLI.",
            "",
        )
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        result = run_active_candidate_source_integrity_controlled(
            args.manifest,
            candidate_bindings=(
                ActiveCandidateSourceBinding(
