...
### A1 Immutable Runtime Projection Publication Contract v1 — Human Gate 4/4

Human Gate **APPROVED / INACTIVE PUBLICATION ONLY**. Se registran cinco decisiones: (1) B52 positiva entregada directamente en memoria dentro de una única ejecución controlada; (2) CLI explícito y autorización humana separada por publicación; (3) root canónico y rutas deterministas por revisión, con idempotencia y rechazo fail-closed de conflictos/duplicados; (4) publicación exclusivamente inactiva, sin activation record, puntero, loader, cambio de autoridad, activación A1 ni modificación de `content/content_tree.json`; (5) `snapshot_revision` se codifica como SHA-256 de sus bytes UTF-8 exactos, sin normalización, en el nombre `sha256-<64 hex minúsculas>.json`, conservando la revisión original en el documento.

El root canónico reutiliza la convención existente `content/<document-family>/...` y queda fijado en `content/runtime-projections/`. Para una proyección elegible, el path exacto es `content/runtime-projections/sha256-<sha256(snapshot_revision.encode("utf-8"))>.json`. El nombre no es la identidad semántica: el documento conserva literalmente `source_snapshot_revision` y el digest de manifest B39. La codificación no aplica trim, lowercase, Unicode normalization, recodificación ni digest del manifest; usa exactamente los bytes UTF-8 de la string original y hex minúsculo. El root y el path son filesystem-locales, absolutos al resolverlos desde el repository root explícito de la ejecución; no hay cwd discovery, registry, DB, red ni rutas caller-provided alternativas.

La publicación recibe B52 positiva in-memory, construye mediante `build_eligible_runtime_content_projection(...)` y serializa exclusivamente con Runtime Documents v1. Antes de visibilidad exige B52 fresca según la ejecución controlada, cardinalidad v1 de una membership/entry, B39 común por identidad `is`, documento válido, parent existente y target inexistente o archivo regular no-symlink. Publica mediante el publisher inmutable canónico: temporal sibling, write completo, flush, file `fsync`, create/link exclusivo y directory `fsync`. Mismos bytes en el mismo path son idempotentes; distinta revisión codificada o distinto contenido para el target, y cualquier duplicado/conflicto de identidad, fallan cerrado sin reemplazo.

Los fallos previos a la visibilidad (evidencia inválida, constructor/serialización, path, parent, target, write, flush, file `fsync`, create/link o conflicto) no publican resultado y limpian el temporal best-effort. Si el directory `fsync` falla después de que el documento sea visible, se informa explícitamente **visible but durability-incomplete**; no se borra, no se hace rollback y no se reintenta automáticamente. La publicación inactiva no cambia la autoridad runtime. Recovery/rollback futuro será una nueva transición record/pointer y permanece fuera.
