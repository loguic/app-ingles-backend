...
Incremento 1 — contexto visual contractual: **CERRADO / PUBLICADO / SINCRONIZADO** mediante `f40abca7abcb36b742404fbfd5e2c7575f3db2f8`. `VisualContext` v3 opcional vincula una o más etapas a un `resource_id` lógico inventariado, con etiqueta accesible, validación v3-only y compatibilidad sin contexto para v2/v3; validación focal/regresión (64 PASS) y repostflight independiente PASS. La única brecha demostrada restante para una candidata ejecutable es la conexión de evaluación oral cualitativa con `evidence states`. Su Incremento 2 tiene preflight técnico **READY_FOR_IMPLEMENTATION**, sin findings BLOCKING ni código iniciado: una captura Direct English estructuralmente elegible puede quedar `needs_review`; solo cobertura cualitativa positiva completa puede acreditar `satisfied`; la automática es auxiliar, B181 se preserva y persistencia/migración serán necesarias, sin API pública ahora. Persistencia de ayudas y adaptación del perfil Direct English v3 quedan condicionadas a candidata real. No se asigna identificador nuevo ni se autoriza candidata, activación, loader o reanudación de B181.
