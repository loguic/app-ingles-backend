...
Entrada A1 canónica — A1 v3 y A1 v4 están publicados. El cierre físico final de los 18/18 assets requeridos de A1-U1 quedó **CLOSED / PUBLISHED / SYNCED** en `3a4c595a34f4d7300fb0ef4fc996fd8855317fe3`; todos están **APPROVED**, instalados y canonicalizados, y ya no quedan assets físicos pendientes. A1 v4 permanece **MEMBER DURABLE / NOT ACTIVE**: no autoriza activar A1, cargar contenido ni reanudar B181. Esta aprobación no completa las human blind reviews formales, no selecciona winner ni cierra Puerta 3. B52 continúa **NOT VERIFIED**, el loader permanece **BLOCKED**, B181 sigue **PAUSED** y `content/content_tree.json` no se modifica.

`ExpectedResourceIdentityCollectionDocumentV1` queda cerrado como contrato reusable: expected identities durable/versionadas/source-bound, serializer determinista, publicación atómica y adquisición read-only que devuelve B45 puro/in-memory. B51/B52 no cambian y expected sigue separado de los bytes observed B49/B50. El primer postflight bloqueó el `resource_id` no-string publicable/no adquirible; el builder ahora lo rechaza sin coerción y el re-postflight final PASS registró 23 focales, 219 regresiones y `git diff --check` PASS.

La materialización real A1 v4 queda cerrada en `content/expected-resource-identities/active-candidate-source-002.json`: 18/18 expected identities source-bound a `active-candidate-source-002`, acquisition V1/B45 y coverage B46↔B45/B47 exacta PASS. El triple cross-check entre bytes actuales, hashes aprobados/documentados y digests durable PASS; trust boundary y postflight independiente PASS con 24 focales y 99 regresiones.

`A1 resource binding adapter v1` queda cerrado: la tabla física canónica A1 del README se adapta bajo `repository_root` explícito a `ResourceBinding` absolutos y al B48 existente. B46/B47 siguen siendo la autoridad del dominio; la integración real entrega 18 bindings, containment y orden B46/B47 PASS, sin resource-byte I/O ni ejecución B49/B50/B51/B52. El postflight independiente PASS registró 10 focales y 101 regresiones.

`A1 source integrity orchestrator v1` queda cerrado: el servicio reusable recibe manifest, bindings candidate/admission, expected document y `repository_root` explícitos, sin locator convention ni cwd inference. Construye un único B39 para las ramas B43 y B51; adquiere B45 una vez, reutiliza B46/B47/B48 y compone B49→B50→B51 sin hashing, lectura de recursos ni comparación manual fuera de B49–B51. Es fail-fast/all-or-nothing, devuelve únicamente B43+B51, no persiste resultados y excluye B52. La integración sintética y postflight independiente PASS registraron 6 focales, 182 regresiones y `git diff --check` PASS. No se ejecutó la cadena A1 real. La siguiente frontera es un preflight read-only para su primera corrida real controlada sobre `active-candidate-source-002`, sin B52, activación, loader ni cierre de Puerta 3.

`A1 source integrity operator CLI v1` queda cerrado: `scripts/engineering/a1_source_integrity_operator.py` es un thin wrapper con siete inputs obligatorios y explícitos, sin defaults, cwd inference ni auto-discovery. Reutiliza los bindings B38/B41, hace una única llamada al orchestrator, calcula same-B39 mediante identidad `is`, emite salida compacta exit 0/1 y crea un report opcional fuera del repo. B52, Git, persistencia, activación, loader y runtime quedan fuera. La integración sintética y postflight independiente PASS registraron 10 focales, 182 regresiones y `git diff --check` PASS. No se ejecutó A1 real. El siguiente paso es un Human Gate separado para autorizar una única corrida real read-only sobre `active-candidate-source-002`.

La corrida real A1 queda reconciliada como **CLOSED / PASS** documental: Human Gate **APPROVED / CONSUMED**, B43 **PASS**, B51 **PASS**, 18 resources verified y same-B39 **PASS** sobre `active-candidate-source-002`. La ejecución válida usó `.venv/bin/python`; la tentativa previa con `/usr/bin/python3` falló antes del orchestrator por dependencia ausente y no produjo evidencia. No se ejecutó B52, no se activó A1, no hubo loader/runtime ni cambios en `content/content_tree.json`, y no fue necesaria instalación de dependencias. A1 v4 sigue **MEMBER DURABLE / NOT ACTIVE**, Puerta 3 **NOT CLOSED**, B52 **NOT VERIFIED**, loader **BLOCKED** y B181 **PAUSED**. La siguiente frontera es el preflight read-only de Puerta 3 y prerequisitos de activación A1.

`A1 B52 controlled execution operator v1` queda implementado y listo para cierre: el wrapper reutiliza el orchestrator B38–B51 exactamente una vez, conserva los aggregates B43/B51 en el mismo proceso y llama B52 exactamente una vez, preservando B39 por identidad `is`. El nuevo CLI separado usa inputs explícitos, no modifica el CLI B38–B51 existente, no persiste verificación y excluye activación, loader/runtime, B181 y `content/content_tree.json`. Focales **8 PASS**, regresiones **192 PASS**, postflight independiente **PASS** y `git diff --check` PASS. No se ejecutó A1 real ni B52 real. Se mantienen A1 v4 **MEMBER DURABLE / NOT ACTIVE**, B52 real **NOT VERIFIED**, Puerta 3 **NOT CLOSED**, `LOADER = BLOCKED` y B181 **PAUSED**. El siguiente objetivo es `human-gate-real-a1-b52-run`.

La corrida real posterior de B52 queda reconciliada documentalmente como **CLOSED / PASS**: Human Gate **APPROVED / CONSUMED**, `active-candidate-source-002`, B43 **PASS**, B51 **PASS**, 18 resources, same-B39 **PASS** y B52 **VERIFIED / PASS**. No se ejecutó activación/runtime ni se modificó `content/content_tree.json`. A1 v4 sigue **MEMBER DURABLE / NOT ACTIVE**, Puerta 3 **NOT CLOSED**, `LOADER = BLOCKED` y B181 **PAUSED**. El siguiente objetivo es el preflight read-only del mecanismo de loader/activación A1; no se repite B52.

TTS WAV normalization profile v1 y TTS Engine Benchmark Protocol v1 están publicados. La ejecución externa anclada a `3e10e97a932f6be27cc05081ba02c555d4611565` está **TÉCNICAMENTE FINALIZADA**. Los doce WAV concretos aprobados directamente como assets físicos A1-U1 quedaron instalados mediante el cierre `cd19bb764177f23754497d914d7d9f3948b7a389`; los dos visuales finales quedaron instalados y canonicalizados mediante el cierre `3a4c595a34f4d7300fb0ef4fc996fd8855317fe3`. Esta aprobación no es la human blind review formal del benchmark, cuya situación sigue **NOT STARTED**, ni selecciona winner, que sigue **NOT SELECTED**. No quedan assets físicos A1-U1 pendientes. La reconciliación privada A/B está **CLOSED / PUBLISHED / SYNCED** en `4d26cf2f3557266b30c26328dbe6f7f76a5e61d9`; Puerta 3 no queda cerrada automáticamente.

El contrato de review-lock quedó **CLOSED / PUBLISHED / SYNCED** históricamente en `7e62908809087f6314d6e17058bbee90d0c967c9`; el public reviewer package/workflow quedó posteriormente **CLOSED / PUBLISHED / SYNCED**. El contrato de B, `Durable / Atomic Human Review Claim Acceptance`, está **CONTRACT APPROVED** y B completo **CLOSED / PUBLISHED / SYNCED** en el commit final `25678e7c07bd249f03ea33dbe815d4d3d0b9e901`: A valida handoff y contexto privado antes de entregar claim y envelope a B; una PK por slot arbitra la aceptación durable, retry y conflicto, conservando review y provenance append-only. Los 15 criterios contractuales están satisfechos. No existe Subpaso 4. Los Subpasos 1, 2 y 3 están **CLOSED / PUBLISHED / SYNCED**; el Subpaso 3 quedó publicado en `1c1c67dbe0e366f876a0d2aac442e94390920cab` con tres carreras PostgreSQL reales, handoffs A auténticos, dos sesiones independientes, `READ COMMITTED`, bloqueo observado con `pg_blocking_pids`, una fila durable por escenario, focales **14 PASS**, regresiones **104 PASS** y postflight independiente **PASS**. La aprobación humana de candidate v4 cubre también su specification embebida cuando es exactamente idéntica a `content/candidates/a1-u1/pedagogical-unit-specification-v3.json`; no se crea approval record ni schema independiente y se preserva el digest v4 `sha256:e75a5c9864adb86a3152e67ab9c97951372e11ed5400b70406f033e6f70b9a8d`. El siguiente objetivo canónico requiere preflight read-only de selección de siguiente frontera canónica. No se autoriza human review, reconciliación de reviews, adjudicación, selección de winner, activación de A1, verificación B52, loader ni reanudación de B181.
La reconciliación vigente establece que B ya está **CLOSED / PUBLISHED / SYNCED** y deja de ser el bloque activo. La specification `pedagogical-unit-specification-v3.json` está **HUMAN APPROVED AS CANONICAL BASIS FOR A1 v4**; candidate v4 permanece **HUMAN APPROVED**. El siguiente objetivo es un preflight read-only específico de Puerta 3 y de los prerequisitos de activación A1. Puerta 3 no ha sido ejecutada ni cerrada; A1 v4 permanece **MEMBER DURABLE / NOT ACTIVE**; B181 **PAUSED**; B52 **NOT VERIFIED**; loader **BLOCKED**.

Human Gate registrado para los dos briefs visuales finales: `scene-help.mp4` debe mostrar, en lenguaje visual ordinario, a una persona usando una máquina que se detiene, un intento fallido de volver a usarla y una mirada hacia otra persona cercana porque necesita ayuda; no debe aparecer inglés escrito ni hablado y debe entenderse como «I need help». `option-farewell.png` debe mostrar a dos personas terminando una interacción, despidiéndose con la mano y comenzando a alejarse; no debe aparecer inglés escrito y debe distinguirse claramente de un saludo inicial. Ambos archivos recibieron aprobación humana final y quedaron instalados/canonicalizados en el cierre `3a4c595a34f4d7300fb0ef4fc996fd8855317fe3`. El estado físico queda en **18/18 APPROVED**.

Human Review Gate del storyboard de `scene-help.mp4`: **APPROVED**. La secuencia aprobada es: una persona usa una máquina; la máquina funciona; la máquina se detiene o falla; la persona lo intenta de nuevo; la persona busca o pide ayuda a otra persona. El archivo final recibió su aprobación humana y quedó canonicalizado en el cierre físico final. `option-farewell.png` también quedó aprobado y canonicalizado en ese mismo cierre.

La siguiente frontera es el preflight read-only específico de Puerta 3 y de los prerequisitos de activación A1. No se declara Puerta 3 cerrada ni A1 activa.
...
## A1 — Publicación real inmutable de activation record

La publicación de `a1-source002-activation-001` fue reportada **PASS** en `content/runtime-activations/sha256-4797505b64d191b46cc708a5f34d12b6ee1f02ca107137be92cdcc8aff52d096.json`. La verificación independiente read-only, separada de ese reporte, adquiere el record contra la proyección publicada y confirma bytes canónicos, esquema v1, vínculos exactos y predecesor nulo. El puntero `content/runtime-active.json` permanece ausente; no hubo activación, loader, cambio de autoridad runtime, modificación de la proyección ni ejecución B52. A1 continúa **NOT ACTIVE**. NEXT = `preparar el incremento de adquisición runtime y protocolo del puntero conforme al plan maestro`; la publicación del puntero permanece **NO AUTORIZADA**.

## A1 — Runtime Active Document Chain v1

Implementación local pendiente de postflight independiente: composición read-only de `active pointer → immutable activation record → immutable runtime projection`, con un único snapshot del puntero y paths deterministas bajo `repository_root` explícito. Reutiliza Runtime Documents v1 y falla cerrado ante documento ausente, no canónico, digest incompatible, identidad pointer/record conflictiva o proyección incompatible. No habilita loader ni cambia autoridad runtime; tampoco crea/publica puntero, ejecuta B52, activa A1 o toca `content/content_tree.json`. Evidencia: 5 focales temporales y 16 regresiones directas PASS. NEXT = `postflight independiente de A1 Runtime Active Document Chain v1`.
