...
Entrada A1 canónica — **HUMAN GATE 1 y HUMAN GATE 2 = APPROVED**. La entrada será una unidad mínima completa, centrada en una Skill pedagógica integrada para alumnado con inglés muy bajo o nulo: responder oralmente a una intención inmediata con contexto visual y audio mediante construcción propia Persona + Acción, pertinente e inteligible, y reutilizarla en una variación cercana con apoyo estrictamente menor. Comprensión, producción y transferencia conservarán evidencias separables; comprensión es preparación/comprobación y no mastery receptivo independiente. Transcript y español quedan como práctica/rescate; la evidencia final deberá producirse sin ellos. La evaluación inicial de pertinencia e inteligibilidad podrá ser humana/externa, con automatización solo de apoyo.

Incremento 1 — contexto visual contractual: **CERRADO / PUBLICADO / SINCRONIZADO** mediante `f40abca7abcb36b742404fbfd5e2c7575f3db2f8`. `VisualContext` v3 opcional vincula una o más etapas a un `resource_id` lógico inventariado, con etiqueta accesible, validación v3-only y compatibilidad sin contexto para v2/v3; validación focal/regresión (64 PASS) y repostflight independiente PASS. Incremento 2 — puente de evaluación oral cualitativa hacia `evidence states`: **TÉCNICAMENTE COMPLETADO — READY_FOR_GIT_CLOSE**. Extiende el requirement v3 con `relevance` e `intelligibility` emparejadas por `production_function`, valida su topología Direct English, persiste historial append-only mediante la migración Alembic `d1842b7f3a91` y promociona `needs_review` a `satisfied` solo tras cobertura positiva completa de una misma identidad y fuente efectiva, a través del acreditador existente. `ExperienceEvidenceState`, completion, automática, B181 y compatibilidad v2/v3 se preservan; 206 pruebas focales/regresión y migración PostgreSQL aislada PASS, postflight independiente PASS. No autoriza candidata, activación, loader, endpoint, frontend ni reanudación de B181; persistencia de ayudas y adaptación Direct English v3 continúan condicionadas a una candidata real.
