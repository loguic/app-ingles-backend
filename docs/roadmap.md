...
El Incremento 1 resolvió técnicamente la representación mínima de contexto visual: `VisualContext` v3 opcional vincula una o más etapas a un `resource_id` lógico inventariado, con etiqueta accesible, validación v3-only y compatibilidad sin contexto para v2/v3. La validación focal/regresión (64 PASS) y el repostflight independiente pasaron. La única brecha demostrada restante para una candidata ejecutable es la conexión de evaluación oral cualitativa con `evidence states`; persistencia de uso de ayudas y adaptación del perfil Direct English v3 quedan condicionadas a necesidad demostrada por una candidata real. Antes de cualquier trabajo posterior solo corresponde cerrar Git este incremento; no se asigna identificador nuevo ni se autoriza candidata, implementación adicional, activación, loader o reanudación de B181.
