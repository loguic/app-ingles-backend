...
La materialización real A1 v4 queda cerrada en `content/expected-resource-identities/active-candidate-source-002.json`: 18/18 expected identities source-bound a `active-candidate-source-002`, acquisition V1/B45 y coverage B46↔B45/B47 exacta PASS. El triple cross-check entre bytes actuales, hashes aprobados/documentados y digests durable PASS; trust boundary y postflight independiente PASS con 24 focales y 99 regresiones. No se ejecutan B51/B52, no se activa A1, no se autoriza loader y no se cierra Puerta 3. La siguiente frontera es un preflight read-only de source-integrity B43/B48–B51.
