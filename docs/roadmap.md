...
Incremento 1 — contexto visual contractual: **CERRADO / PUBLICADO / SINCRONIZADO** mediante `f40abca7abcb36b742404fbfd5e2c7575f3db2f8`. `VisualContext` v3 opcional vincula una o más etapas a un `resource_id` lógico inventariado, con etiqueta accesible, validación v3-only y compatibilidad sin contexto para v2/v3; validación focal/regresión (64 PASS) y repostflight independiente PASS. Incremento 2 — puente de evaluación oral cualitativa hacia `evidence states`: **CERRADO / PUBLICADO / SINCRONIZADO** mediante `1485309a80c63c1786b831703c89920e68c9b70d`; conserva `ExperienceEvidenceState`, completion, automática, B181 y compatibilidad v2/v3, con cabeza Alembic `d1842b7f3a91`, 206 pruebas focales/regresión, migración PostgreSQL aislada y postflight independiente PASS. El preflight de candidata A1 confirma `a1-u1` reutilizable, `contract_version="3.0"`, sustitución de la Skill y contenido histórico, y deja lesson ID y `candidate_revision` por decidir; queda **BLOCKED** únicamente hasta aprobar una `PedagogicalUnitSpecification` aislada, concreta y versionada. No autoriza candidata, activación, loader, endpoint, frontend ni reanudación de B181.
