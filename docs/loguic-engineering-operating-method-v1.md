...
- `block_workflow.py` es el orquestador canónico del tramo determinista del Closure Gate para bloques futuros, siempre después de que postflight independiente, documentación semántica, scope/allowlist y mensaje hayan quedado aprobados; su `--checkpoint-prepare-format {markdown,compact}` afecta exclusivamente a la representación del `prepare` final: `markdown` por defecto ejecuta `conversation_checkpoint.py prepare` y `compact` opt-in ejecuta una sola vez `conversation_checkpoint.py prepare --format compact`. No interpreta `compact-v1`, preserva stdout/stderr y no relanza automáticamente Markdown ante FAIL; el checkpoint inicial, orden, fallo cerrado, partial closure y el resto de garantías del Closure Gate no cambian;
- `validation_recipe.py approved-v1` es un runner read-only separado para una receta explícitamente aprobada: recibe rutas focal/regression decididas antes por ChatGPT/Codex, las pasa una sola vez a pytest sin descubrirlas y luego compone diff check, validación de estado y checkpoint compacto. Su PASS no acredita readiness, postflight, scope ni autorización de cierre; no usa shell ni invoca helpers Git mutantes, `git_close.py` o `block_workflow.py`;
...
El Closure Gate canónico, cuando esté autorizado, ejecuta `checkpoint validation → block_close.py → git_close.py → conversation_checkpoint.py prepare`. `block_workflow.py` automatiza exclusivamente ese tramo determinista para Closure Gates posteriores: recibe los argumentos técnicos de `block_close.py`, branch, upstream, mensaje, allowlist exacta y timeout; ejecuta cada helper como proceso separado, falla cerrado y no avanza después de un fallo. No decide readiness ni reconstruye lógica interna de los helpers.
