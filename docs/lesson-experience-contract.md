...
## Extensión v3 — puente de evaluación oral cualitativa Direct English

`ExternalReviewRequirement` conserva las dimensiones históricas `intention_understanding` y `contingent_response` con `production_function=null`. En v3 añade, de forma opcional, `relevance` e `intelligibility`, cada una ligada explícitamente a `guided`, `expanded` o `transfer`. La pareja `relevance` + `intelligibility` es obligatoria por función declarada; la unicidad es `(production_function, dimension)`. Las extensiones cualitativas no cambian el contrato v2 ni las declaraciones históricas B181.

La validación Direct English admite estos requirements solo en evidencia productiva de una experiencia Direct English v3 y exige que la función figure realmente entre las capturas de la conversación vinculada. El contrato no obliga a revisar ninguna función: una futura candidata decidirá cuáles declara. La evaluación automática, semántica o fonética no satisface estos requirements ni adquiere autoridad pedagógica.

Una captura Direct English estructuralmente inelegible sigue `pending`; una elegible sin requirements cualitativos conserva `satisfied`; una elegible con requirements cualitativos queda `needs_review`. La fuente pedagógica efectiva continúa siendo el mismo `DirectEnglishConstructionAttempt`: una review autoriza la promoción, no crea una fuente de evidencia independiente.

Las revisiones se conservan append-only por enlace de producción, sin duplicar identidad de intento, evidencia o función. Un batch valida una sola identidad exacta de revisor `(source_type, source_id, source_version)`, se persiste sin commit intermedio y se promueve en la misma transacción únicamente si esa identidad aporta cobertura `positive` completa de todas las parejas declaradas para la misma experiencia, intento Direct English, evidencia y fuente efectiva. `negative`, `pending`, cobertura parcial, mezcla de reviewers/versiones o reviews de un intento sustituido no acreditan; una `positive` histórica de la misma identidad y fuente efectiva puede contar. `satisfied` permanece monotónico.

La promoción invoca exclusivamente el acreditador autoritativo existente: completion sigue dependiendo solo de que todas las evidencias requeridas estén `satisfied`. No se introducen API pública, UI de reviewer, candidata, scorer automático autoritativo, progreso, mastery ni retención. La persistencia y runtime se cerraron técnicamente con cabeza Alembic `d1842b7f3a91`, 206 pruebas focales/regresión y migración PostgreSQL aislada PASS; B181 permanece independiente y pausado.
