...
Estado: microampliación contractual aprobada y documentada; sin implementación, tests ni cambio de configuración Git. Es un microbloque separado, no slice 38. Se origina en el incidente de transporte de slice 37: el push SSH falló, `git_close.py` preservó el commit local y reportó ahead=1/behind=0; SSH por puertos 22 y 443 siguió cerrado, mientras HTTPS permitió `push --dry-run`, push y fetch explícitos sin modificar `origin` ni claves.

`--publish-url` será una opción opcional, explícita y HTTPS caller-provided. Sin ella, el flujo v1 actual permanece intacto. `--upstream` sigue siendo la referencia canónica configurada; el caller declara que la URL es un transporte alternativo de ese upstream. La v1 no afirma prueba de identidad criptográfica entre URL y remote: exige antes del commit que el OID del ref remoto consultado por URL coincida con `@{upstream}` y, después del push, que ese ref sea exactamente `HEAD`. Solo entonces actualizará la ref remota local mediante fetch explícito desde esa URL y aplicará el invariante final existente: index/worktree limpios y ahead/behind `0/0` frente a `@{upstream}`.

La URL no cambia `origin`, upstream, remote, claves ni otra configuración persistente. No hay fallback automático SSH→HTTPS, retry, force, rollback, `set-upstream` ni múltiples destinos. Push fallido conserva el commit local; si la publicación por URL ya ocurrió pero falla su verificación, fetch o sincronización final, el helper falla explícitamente sin revertir una publicación posiblemente visible. El próximo paso es implementar y probar solo esta extensión de tooling antes de retomar acquisition/source integrity.
