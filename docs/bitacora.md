...
Permanecen como límites no bloqueantes fuera de v1 la simulación directa de una carrera entre stage y commit y el hardening de remote names que comiencen por `-`. El primer uso real queda preparado, todavía no ejecutado: Bash invocará el helper con allowlist exacta `docs/estado-operativo.md` y `docs/bitacora.md`, y mensaje `docs finalize deterministic git close helper`; solo un resultado exitoso demostrará precheck → stage exacto → verify → commit → push → final sync verify. Después corresponde retomar la slice 37 con el preflight de representación física explícita de `ActiveCandidateSourceSnapshot`/source y atomicidad física, antes de source integrity, adquisición y loader.
