...
Estado: **TÉCNICAMENTE COMPLETADO / READY_FOR_GIT_CLOSE**. Se añade `VisualContext` a `LessonExperience` como lista opcional vacía por defecto. Cada contexto v3 declara `id`, `resource_id` lógico, `accessibility_label` y una o más `stage_ids`; valida campos escalares no vacíos, IDs de contexto únicos, stages no vacíos/únicos/existentes en la misma experiencia y rechazo de contextos poblados en v2. El recurso debe figurar en `PedagogicalUnitCandidate.required_resource_ids`; no se introducen URL, path, bytes, binding físico, asset, MIME, descarga ni telemetría.
