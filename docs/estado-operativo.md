...
Actualizado: 2026-09-08T20:28:48+02:00
...
Resolver primero la brecha contractual de media de `VisualContext` antes de corregir o revisar `a1-u1-candidate-v3`: se requieren representación verificable de tipo de recurso (imagen/microvídeo) y política de reproducción (`autoplay` único y `replay`). No iniciar endpoint, frontend, loader, admission, publication, membership, activación ni B181. A1 L1 v3 permanece inactiva, A1-U1 `pending / non-member` y `LOADER = BLOCKED`.
