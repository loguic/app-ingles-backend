...
Actualizado: 2026-09-11T19:35:20+02:00
...
Revisión focal A1-U1 Candidate v4 = **HUMAN REVIEW APPROVED / ADMISSION ADMITTED Y VERIFIED** en `content/candidates/a1-u1/pedagogical-unit-candidate-v4.json`, con `candidate_revision=a1-u1-candidate-v4`, único delta canónico `help` en-GB `/hɛlp/` → `/help/` y digest confirmado `sha256:e75a5c9864adb86a3152e67ab9c97951372e11ed5400b70406f033e6f70b9a8d`. El AdmissionRecord físico `content/admissions/a1-u1/adm-a1-u1-002.json` registra `admission_id=adm-a1-u1-002`, `reviewer_id=reviewer-human-001` y decisión `admitted`; `verify_candidate_admission(...)` es positivo. V4 sigue **NOT MEMBER / NOT ACTIVE**: esta admission no autoriza membership ni snapshot. El postflight inicial detectó y bloqueó una incorporación retroactiva de la coherencia IPA al validador global; la corrección conserva esa prevención como comprobación focal de v4, restaura la validación y B43 positivos de v3 y deja pendiente un diseño posterior de prevención general versionada. Candidate v3, `adm-a1-u1-001` y `active-candidate-source-001` permanecen byte-identical como historia. El lote TTS queda congelado hasta nuevo membership/snapshot v4. Los cuatro assets visuales ya aprobados siguen reutilizables byte-identical; PHYSICAL ASSETS = **4/18 APPROVED**, WAV = **0**, B52 = **NOT VERIFIED**, `LOADER = BLOCKED` y B181 = **PAUSED**.
...
Producir únicamente los 14 assets A1-U1 restantes queda congelado hasta nuevo membership/snapshot v4. La revisión humana y Admission de Candidate A1-U1 v4 están aprobadas; el siguiente frente requiere preparar membership y snapshot separados. No producir TTS, crear snapshot v4, iniciar B52, activar contenido, tocar frontend/loader ni reanudar B181. V3 permanece histórica; v4 está `human approved / admitted / not member / not active`.
